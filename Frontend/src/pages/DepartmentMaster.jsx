...
function AddOrEdit({ onSave, onCancel, editData, userRole, selectedCompanyId, selectedCompanyName }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!departmentName || !departmentAckr) return toast.error(t("pleaseFillAllFields"));
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm">
      <div className="relative max-w-xl w-full bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
        ...
        <h2 className="text-2xl font-bold text-gray-800 text-center mb-6">
          {editData ? t("editDepartment") : t("addNewDepartment")}
        </h2>
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            <label className="block font-medium text-gray-700 mb-2">{t("departmentName")}</label>
            <input
              type="text"
...
function DepartmentMaster({ userRole, selectedCompanyId, selectedCompanyName }) {
  ...
  const handleSave = async (departmentData, departmentId) => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: departmentId ? t("departmentUpdated") : t("departmentAdded"),
        text: departmentId ? t("departmentUpdated") : t("departmentAdded"),
      });
    } catch (err) {
      Swal.fire({
        icon: "error",
        title: departmentId ? t("departmentUpdateFailed") : t("departmentAddFailed"),
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire({
            title: t("error"),
            text: t("failedToDeleteDepartment"),
            icon: "error",
          });
        }
      }
  ...
  return (
    <div className="h-full flex flex-col px-6">
      <div className="flex justify-between items-center mb-4">
        ...
        >
          <Plus size={18} /> {t("addNewDepartment")}
        </button>
      </div>
          ...
          <tbody>
            {filteredData.map((d) => (
              <tr key={d.departmentId} className="border-t hover:bg-gray-50">
                ...
                {!selectedCompanyId && <td className="py-2 px-4">{getCompanyAcronym(d.companyId)}</td>}
            ...
            {filteredData.length === 0 && (
              <tr>
                <td colSpan={selectedCompanyId ? 4 : 5} className="text-center py-4 text-gray-500">
