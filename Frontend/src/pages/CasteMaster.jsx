...
function AddOrEdit({ onSave, onCancel, editData }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSave(casteData, editData?.caste_id);
  };
...
function CasteMaster() {
  ...
  const handleSave = async (casteData, caste_id) => {
    try {
      if (caste_id) {
        await axios.put(`http://localhost:5000/api/castes/${caste_id}`, casteData);
      } else {
        await axios.post("http://localhost:5000/api/castes", casteData);
      }
      fetchCastes();
      setShowForm(false);
      setEditData(null);
  ...
  const handleDelete = async (caste_id) => {
    try {
      await axios.delete(`http://localhost:5000/api/castes/${caste_id}`);
      fetchCastes();
    } catch (err) {
    ...
    <div className="min-h-screen p-6 flex flex-col justify-center align-center">
      ...
      <div className="overflow-y-auto" style={{ maxHeight: "260px" }}>
        <table className="w-full text-left border border-gray-300">
          ...
          <tbody>
            {filteredData.map((c) => (
              <tr key={c.caste_id} className="border-t">
                <td className="py-2 px-4">{c.caste_id}</td>
                ...
                <td className="py-2 px-4 flex gap-2">
                  ...
                  <button
                    ...
                    onClick={() => handleDelete(c.caste_id)}
                  >
                    <Trash />
                  </button>
                </td>
