...
function AddOrEdit({ onSave, onCancel, editData, userRole, selectedCompanyId, selectedCompanyName }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!busNumber || !busDriverName || !busRouteDetails) return toast.error(t("pleaseFillAllFields"));
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm">
      <div className="max-w-xl w-full bg-white rounded-2xl shadow-xl p-6 relative">
        ...
        <h2 className="text-xl font-bold text-gray-800 text-center mb-4">
          {editData ? t("editBusDetails") : t("addNewBus")}
        </h2>

        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block font-medium text-gray-700 mb-1">{t("busNumber")}</label>
            <input
              type="text"
              value={busNumber}
          ...
          <div>
            <label className="block font-medium text-gray-700 mb-1">{t("driverName")}</label>
            <input
              type="text"
              value={busDriverName}
          ...
          <div>
            <label className="block font-medium text-gray-700 mb-1">{t("routeDetails")}</label>
            <input
              type="text"
...
function BusMaster({ selectedCompanyId, selectedCompanyName }) {
  ...
  const fetchBuses = async () => {
    try {
      ...
      toast.error(t("errorFetchingBuses"));
    }
  };
  ...
  const handleSave = async (busData, busId) => {
    try {
      if (busId) {
        await axios.put(`http://localhost:5000/api/buses/${busId}`, busData, { headers: { Authorization: `Bearer ${token}` } });
      } else {
        await axios.post(`http://localhost:5000/api/buses`, busData, { headers: { Authorization: `Bearer ${token}` } });
      }
      Swal.fire({
        icon: "success",
        title: busId ? t("busUpdated") : t("busAdded"),
        text: busId ? t("busUpdated") : t("busAdded"),
      });
      fetchBuses();
      setShowForm(false);
      setEditData(null);
      ...
      Swal.fire({
        icon: "error",
        title: busId ? t("busUpdateFailed") : t("busAddFailed"),
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          Swal.fire(t("deleted"), t("busDeleted"), "success");
          fetchBuses();
        } catch (err) {
          console.error("Error deleting bus:", err);
          Swal.fire(t("error"), t("failedToDeleteBus"), "error");
        }
      }
    });
  };
          ...
          <table className="w-full border-collapse">
            <thead className="sticky top-0">
              <tr className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                <th className="py-3 px-4 text-left font-semibold">{t("busNumber")}</th>
                <th className="py-3 px-4 text-left font-semibold">{t("driverName")}</th>
                <th className="py-3 px-4 text-left font-semibold">{t("routeDetails")}</th>
              ...
              {filteredData.length > 0 ? filteredData.map(b => (
                <tr key={b.busId} className="hover:bg-gray-50 transition">
                  ...
                  <td className="py-3 px-4 flex justify-center gap-2">
                    <button className="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-md shadow" onClick={() => handleEdit(b)} title={t("edit")}>
                      <Pencil size={16} />
                    </button>
                    <button className="bg-red-500 hover:bg-red-600 text-white p-2 rounded-md shadow" onClick={() => handleDelete(b.busId)} title={t("delete")}>
                      <Trash size={16} />
                    </button>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={selectedCompanyId ? 4 : 5} className="py-16 text-center text-gray-500">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="font-medium">{t("noBusesFound")}</p>
                      <p className="text-sm text-gray-400 mt-1">{t("adjustSearchOrAddBus")}</p>
                    </div>
                  </td>
                </tr>
              )}
