...
function AddOrEditCategory({ onSave, onCancel, editData }) {
  ...
  const handleSubmit = (e) => {
    ...
    onSave(categoryData, editData?.category_id);
  };
...
function CategoryMaster() {
  ...
  const handleSave = async (categoryData, category_id) => {
    try {
      if (category_id) {
        await axios.put(`http://localhost:5000/api/categories/${category_id}`, categoryData);
      } else {
          ...
          <tbody>
            {filteredData.map((c) => (
              <tr key={c.category_id} className="border-t">
                <td className="py-2 px-4">{c.category_id}</td>
