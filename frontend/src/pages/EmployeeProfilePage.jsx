...
const SECTION_META = {
  "Identification & Core":   { emoji: "🪪", ring: "border-sky-200",    header: "bg-sky-50",    badge: "bg-sky-100 text-sky-600",    dot: "bg-sky-400" },
  "Basic Information":       { emoji: "👤", ring: "border-violet-200",  header: "bg-violet-50",  badge: "bg-violet-100 text-violet-600",  dot: "bg-violet-400" },
  "Contact Information":     { emoji: "📬", ring: "border-blue-200",     header: "bg-blue-50",     badge: "bg-blue-100 text-blue-600",     dot: "bg-blue-400" },
  "Current Address":         { emoji: "🏠", ring: "border-amber-200",   header: "bg-amber-50",   badge: "bg-amber-100 text-amber-600",   dot: "bg-amber-400" },
  "Permanent Address":       { emoji: "📍", ring: "border-emerald-200", header: "bg-emerald-50", badge: "bg-emerald-100 text-emerald-600", dot: "bg-emerald-400" },
  "Employment Information":  { emoji: "💼", ring: "border-indigo-200",  header: "bg-indigo-50",  badge: "bg-indigo-100 text-indigo-600",  dot: "bg-indigo-400" },
  "Shift & Attendance":      { emoji: "🕐", ring: "border-pink-200",    header: "bg-pink-50",    badge: "bg-pink-100 text-pink-600",    dot: "bg-pink-400" },
  "Salary & Bank":           { emoji: "💰", ring: "border-green-200",   header: "bg-green-50",   badge: "bg-green-100 text-green-600",   dot: "bg-green-400" },
  "Transport":               { emoji: "🚌", ring: "border-orange-200",  header: "bg-orange-50",  badge: "bg-orange-100 text-orange-600",  dot: "bg-orange-400" },
  "Documents":               { emoji: "📄", ring: "border-purple-200",  header: "bg-purple-50",  badge: "bg-purple-100 text-purple-600",  dot: "bg-purple-400" },
  "Academic & Research":     { emoji: "🎓", ring: "border-teal-200",    header: "bg-teal-50",    badge: "bg-teal-100 text-teal-600",    dot: "bg-teal-400" },
  "Exit":                    { emoji: "🚪", ring: "border-rose-200",    header: "bg-rose-50",    badge: "bg-rose-100 text-rose-600",    dot: "bg-rose-400" },
};
...
const inputCls = "w-full px-3.5 py-2.5 rounded-xl border border-gray-200 bg-white text-gray-800 text-sm placeholder-gray-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-50 shadow-sm";
...
export default function EmployeeProfilePage() {
  ...
  const renderField = (field) => {
    ...
    if (field.name === "profilePhoto") {
      return (
        <div key={field.name} className="space-y-2">
          ...
          <input id={field.name} name={field.name} type="file" accept="image/*" onChange={onPhotoFileChange} className={inputCls} />
          {photoPreview ? (
            <img src={photoPreview} alt="Profile preview" className="h-20 w-20 rounded-xl border border-gray-200 object-cover shadow-sm" />
          ) : null}
        </div>
      );
    }
    ...
    if (field.type === "checkbox") {
      return (
        <label key={field.name} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 hover:border-blue-300 hover:bg-blue-50 cursor-pointer transition-all shadow-sm">
          <input {...commonProps} type="checkbox" checked={Boolean(value)} onChange={onChange} className="w-4 h-4 rounded accent-blue-500 cursor-pointer" />
    ...
    return (
      <div key={field.name}>
        <label htmlFor={field.name} className={labelCls}>{field.label}</label>
        <input {...commonProps} type={field.type || "text"} value={field.type === "number" ? value ?? "" : value || ""} className={inputCls} />
      </div>
    );
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8 pb-6 border-b border-gray-200">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-md shadow-blue-200">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                Staff ID:{" "}
                <span className="text-blue-600 font-semibold">{staffId || "N/A"}</span>
                {" "}&nbsp;|&nbsp; User Number:{" "}
                <span className="text-indigo-600 font-semibold">{userNumber || "N/A"}</span>
              </p>
            </div>
          </div>
          <button
            type="button"
            onClick={loadData}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-200 bg-white hover:bg-blue-50 hover:border-blue-300 text-gray-500 hover:text-blue-600 text-sm font-semibold transition-all shadow-sm"
          >
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          {FIELD_SECTIONS.map((section) => {
            const meta = SECTION_META[section.title] || { emoji: "📋", ring: "border-gray-200", header: "bg-gray-50", badge: "bg-gray-100 text-gray-600", dot: "bg-gray-400" };
            const isOpen = openSections.has(section.title);

            return (
              <div
                key={section.title}
                className={`rounded-2xl border bg-white overflow-hidden transition-all duration-200 shadow-sm hover:shadow-md ${isOpen ? meta.ring : "border-gray-200"}`}
              >
                {/* Section Header */}
                <button
                  type="button"
                  onClick={() => toggleSection(section.title)}
                  className={`w-full flex items-center gap-3 px-5 py-4 text-left transition-colors ${isOpen ? meta.header : "hover:bg-gray-50"}`}
                >
          ...
          <div className="flex justify-end pt-4 border-t border-gray-200">
            <button
              type="submit"
              disabled={saving}
              className="inline-flex items-center gap-2.5 px-7 py-3 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-400 hover:to-indigo-500 text-white font-bold text-sm shadow-md shadow-blue-200 hover:shadow-blue-300 transition-all hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:translate-y-0"
            >
