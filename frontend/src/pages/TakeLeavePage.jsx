...
const inputCls = "w-full px-3.5 py-2.5 rounded-xl border border-white/10 bg-white/[0.04] text-slate-200 text-sm placeholder-slate-600 outline-none focus:border-violet-500/50 focus:bg-violet-500/[0.04] focus:ring-1 focus:ring-violet-500/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed";
...
export default function TakeLeavePage({ empId, companyId }) {
  ...
  const validateForm = (targetStatus) => {
    ...
    if (formData.hasDocuments) {
      if (parseDocumentPaths(formData.documentPathsInput).length === 0) {
        return { ok: false, message: "Add at least one document path when documents are enabled" };
      }
    }
    const selectedLeaveBalance = leaveBalances.find(
      (b) => Number(b.leaveTypeId) === Number(formData.leaveTypeId)
    );
    if (!selectedLeaveBalance) {
      return { ok: false, message: "Selected leave type balance is not available" };
    }
    if (Number(totalDays) > Number(selectedLeaveBalance.available || 0)) {
      return { ok: false, message: "Not enough leave balance" };
    }
  ...
  const submitRequest = async (targetStatus, options = {}) => {
    ...
    if (!skipConfirm) {
      const confirm = await Swal.fire({
        ...
        html: isSubmit
          ? `<p style="color:#94a3b8;font-size:14px;">Your request for <strong style="color:#f1f5f9">${totalDays} day(s)</strong> will be sent for approval.</p>`
          : `<p style="color:#94a3b8;font-size:14px;">Your leave request will be saved as a draft.</p>`,
        ...
        background: "#0f172a",
        color: "#f1f5f9",
        confirmButtonColor: isSubmit ? "#7c3aed" : "#f59e0b",
    ...
    try {
      ...
      const finalMessage =
        /insufficient|exceed|balance/i.test(String(apiMessage)) ? "Not enough leave balance" : apiMessage;
  ...
  // Cycle through accent colors for balance cards
  const cardAccents = [
    { border: "border-cyan-500/25", bg: "hover:bg-cyan-500/[0.05]", num: "text-cyan-400", top: "bg-gradient-to-r from-cyan-500 to-sky-500" },
    { border: "border-violet-500/25", bg: "hover:bg-violet-500/[0.05]", num: "text-violet-400", top: "bg-gradient-to-r from-violet-500 to-indigo-500" },
    { border: "border-amber-500/25", bg: "hover:bg-amber-500/[0.05]", num: "text-amber-400", top: "bg-gradient-to-r from-amber-500 to-orange-500" },
    { border: "border-emerald-500/25", bg: "hover:bg-emerald-500/[0.05]", num: "text-emerald-400", top: "bg-gradient-to-r from-emerald-500 to-teal-500" },
    { border: "border-rose-500/25", bg: "hover:bg-rose-500/[0.05]", num: "text-rose-400", top: "bg-gradient-to-r from-rose-500 to-pink-500" },
  ];
    ...
    <div className="min-h-full bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-6 sm:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4 pb-6 border-b border-white/10">
          <div className="w-13 h-13 w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-violet-500/25 shrink-0">
            <FileText className="w-6 h-6 text-white" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Leave Request</h1>
            <p className="text-xs text-slate-500 mt-0.5">Fill all required details and submit for approval</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Available Leave Balances</h2>
          </div>

          {loading ? (
            <div className="flex items-center gap-2 text-slate-500 text-sm">
              <Loader2 className="w-4 h-4 animate-spin text-violet-400" /> Loading balances...
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {leaveBalances.map((b, i) => {
                ...
                return (
                  <div
                    key={b.leaveTypeId}
                    className={`relative rounded-xl border bg-white/[0.03] overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg ${accent.border} ${accent.bg}`}
                  >
                    <div className={`h-0.5 w-full ${accent.top}`} />
                    <div className="p-4">
                      <p className="text-sm font-bold text-slate-200 mb-3">{b.name}</p>
                      ...
                      <div className="flex gap-4">
                        <span className="text-xs text-slate-600">Used: <span className="text-slate-400 font-semibold">{b.used}</span></span>
                        <span className="text-xs text-slate-600">Allocated: <span className="text-slate-400 font-semibold">{b.allocated}</span></span>
                      </div>
                    </div>
                  </div>
                );
        ...
        >
          <div className="flex items-center gap-2 mb-6 pb-4 border-b border-white/10">
            ...
            <h2 className="text-sm font-bold text-slate-200">Request Details</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            {/* Leave Category */}
            <div>
              <label className={labelCls}>Leave Category <span className="text-rose-400">*</span></label>
            ...
            <div>
              <label className={labelCls}>
                <span className="flex items-center gap-1.5"><Calendar className="w-3 h-3" /> Start Date <span className="text-rose-400">*</span></span>
              </label>
            ...
            <div>
              <label className={labelCls}>
                <span className="flex items-center gap-1.5"><Calendar className="w-3 h-3" /> End Date <span className="text-rose-400">*</span></span>
              </label>
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-3 w-full px-4 py-3 rounded-xl border border-white/10 bg-white/[0.03] hover:border-violet-500/25 hover:bg-violet-500/[0.04] cursor-pointer transition-all">
                <input
                  type="checkbox"
                  name="hasDocuments"
                ...
                />
                <span className="text-sm text-slate-400 font-medium">Supporting documents attached</span>
              </label>
            </div>

            {/* Reason - full width */}
            <div className="md:col-span-2">
              <label className={labelCls}>Reason <span className="text-rose-400">*</span></label>
              <textarea
                name="reason"
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                onChange={onChange}
                rows={2}
                placeholder="Where can we reach you during leave?"
                className={`${inputCls} resize-y`}
              />
            </div>

            {/* Document Paths - conditional */}
            {formData.hasDocuments && (
              <div className="md:col-span-2">
                <label className={labelCls}>Document Paths <span className="text-rose-400">*</span></label>
                <textarea
                  name="documentPathsInput"
                  value={formData.documentPathsInput}
                  onChange={onChange}
                  rows={3}
                  placeholder={"One file path per line\nuploads/leave/medical-cert-1.pdf"}
                  className={`${inputCls} resize-y`}
                />
                <p className="text-xs text-slate-600 mt-1.5 pl-1">Enter one file path per line</p>
              </div>
            )}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-5 border-t border-white/10">
            ...
            {/* Day counter */}
            <div className="flex items-center gap-3 px-5 py-3 rounded-xl border border-violet-500/25 bg-violet-500/[0.06]">
              <TrendingDown className="w-5 h-5 text-violet-400" />
              <div>
                <span className="text-3xl font-bold text-violet-400 leading-none">{totalLeavesAvailed}</span>
                <span className="text-xs text-slate-500 ml-2">total leaves availed (current year)</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-white/10 bg-white/[0.04] hover:bg-amber-500/10 hover:border-amber-500/30 text-slate-400 hover:text-amber-400 font-semibold text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
