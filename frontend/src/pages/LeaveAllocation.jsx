...
import { Download, FileBarChart, Upload } from 'lucide-react';
  ...
  return (
    <div className="h-full flex flex-col px-6">
        ...
        actions={
          <>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl shadow-md font-semibold text-sm whitespace-nowrap transition-all hover:-translate-y-0.5"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-slate-700 hover:bg-slate-800 text-white px-4 py-2.5 rounded-xl shadow-md font-semibold text-sm whitespace-nowrap transition-all hover:-translate-y-0.5"
            >
      ...
      <div className="mb-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div>
          <FilterDropdown
            ...
            onToggleValue={(value) =>
              setFilters((p) => ({
                ...p,
                departmentIds: toggleValue(p.departmentIds, String(value)),
              }))
            }
                  ...
                  <input
                    ...
                    onChange={() =>
                      setFilters((p) => ({
                        ...p,
                        departmentIds:
                          p.departmentIds.length === departments.length
                            ? []
        ...
        <div>
          <FilterDropdown
            ...
            onToggleValue={(value) =>
              setFilters((p) => ({
                ...p,
                roleNames: toggleValue(p.roleNames, value),
              }))
            }
              ...
              roleOptions.length > 0 ? (
                <label className="flex items-center gap-2 text-sm text-slate-700 pb-2 border-b border-slate-100 mb-2">
                  <input
                    type="checkbox"
                    className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500"
                    checked={filters.roleNames.length === roleOptions.length}
                    onChange={() =>
                      setFilters((p) => ({
                        ...p,
                        roleNames: p.roleNames.length === roleOptions.length ? [] : [...roleOptions],
                      }))
                    }
                  />
        ...
        <div>
          <FilterDropdown
            ...
            selectedValues={filters.experienceBands}
            onToggleValue={(value) =>
              setFilters((p) => ({
                ...p,
                experienceBands: toggleValue(p.experienceBands, value),
              }))
            }
              ...
              true ? (
                <label className="flex items-center gap-2 text-sm text-slate-700 pb-2 border-b border-slate-100 mb-2">
                  <input
                    type="checkbox"
                    className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-2 focus:ring-indigo-500"
                    checked={filters.experienceBands.length === 4}
                    onChange={() =>
                      setFilters((p) => ({
                        ...p,
                        experienceBands:
                          p.experienceBands.length === 4 ? [] : ['lt1', '1to3', '3to5', '5plus'],
                      }))
                    }
                  />
      ...
      {selectedEmployeeIds.length > 0 && (
        <div className="mb-4 text-sm text-slate-600 bg-indigo-50 px-4 py-2 rounded-lg border border-indigo-100">
      ...
      >
        {sortedEmployees.map((emp) => (
          <tr
            key={emp.staffId}
            className={`border-t hover:bg-gray-50 ${selectedEmployeeIds.includes(emp.staffId) ? 'bg-indigo-50' : ''}`}
          >
            ...
            <td className="py-3 px-4 text-slate-700 font-medium">{employeeName(emp)}</td>
            <td className="py-3 px-4 text-slate-600">{getEmployeeRoleName(emp) || '-'}</td>
            <td className="py-3 px-4 text-slate-600">{designationMap.get(String(emp.designationId || '')) || '-'}</td>
            <td className="py-3 px-4 text-slate-600">{gradeMap.get(String(emp.employeeGradeId || '')) || '-'}</td>
            <td className="py-3 px-4 text-slate-600">{emp.department?.departmentName || '-'}</td>
          </tr>
        ))}
      </MasterTable>
