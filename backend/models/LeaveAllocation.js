...
export default (sequelize) => {
  const LeaveAllocation = sequelize.define('LeaveAllocation', {
    ...
    leavePolicyId: {  // ← Added (optional but highly recommended)
      type: DataTypes.INTEGER,
      allowNull: true,
      references: {
      ...
      onDelete: 'SET NULL',
      comment: 'The policy from which this allocation was derived',
    },
    ...
    maxCarryForwardLimit: {
      type: DataTypes.DECIMAL(10, 2),
      allowNull: true,
      comment: 'Maximum days that can be carried forward (per-employee override)',
    },
  ...
  //   LeaveAllocation.sync({ alter: true })
  //     .then(() => console.log('LeaveAllocation table synced successfully'))
  //     .catch((err) => console.error('Error syncing LeaveAllocation table:', err));
  // }
